...
import { requireActiveSubscription, requireUserLimit, requireTeacherLimit } from '../middleware/subscriptionMiddleware';

const router = Router();

router.use(authenticateToken);
router.use(requireActiveSubscription); // Check subscription on all user routes
...
router.post('/', 
  ...
  requireUserLimit, // Check user limit before creating
