...
const assignSubjectTeacher = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const [classExists, subjectExists, teacherExists] = yield Promise.all([
            prisma_1.prisma.class.findUnique({ where: { id: classId } }),
            prisma_1.prisma.subject.findUnique({ where: { id: subjectId } }),
            prisma_1.prisma.user.findUnique({ where: { id: teacherId, role: { in: ['TEACHER', 'SUPER_ADMIN'] } } }),
        ]);
        if (!classExists)
        ...
        const assignment = yield prisma_1.prisma.teacherSubject.upsert({
            where: {
                classId_subjectId: {
                    classId,
                    subjectId,
...
const getClassSubjectTeachers = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const assignments = yield prisma_1.prisma.teacherSubject.findMany({
            where: { classId },
            include: {
                teacher: {
                    select: {
