    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (!settings) {
                ...
                if (logId)
                    yield (0, communicationLogService_1.updateCommunicationLogStatus)(logId, 'FAILED', 'No school settings found');
                return false;
            }
            ...
            if (!emailEnabled) {
                ...
                if (logId)
                    yield (0, communicationLogService_1.updateCommunicationLogStatus)(logId, 'FAILED', 'Email notifications disabled');
                return false;
            }
            if (!settings.smtpHost || !settings.smtpUser || !settings.smtpPassword) {
                console.log('SMTP settings not configured');
                if (logId)
                    yield (0, communicationLogService_1.updateCommunicationLogStatus)(logId, 'FAILED', 'SMTP not configured');
                return false;
            }
            ...
            if (logId)
                yield (0, communicationLogService_1.updateCommunicationLogStatus)(logId, 'FAILED', error.message || 'Unknown error');
            return false;
        }
    });
}
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (!smsEnabled) {
                ...
                if (logId)
                    yield (0, communicationLogService_1.updateCommunicationLogStatus)(logId, 'FAILED', 'SMS notifications disabled');
                return false;
            }
            if (!settings.smsProvider || !settings.smsApiKey) {
                console.log('SMS settings not configured');
                if (logId)
                    yield (0, communicationLogService_1.updateCommunicationLogStatus)(logId, 'FAILED', 'SMS not configured');
                return false;
            }
            ...
            switch (settings.smsProvider.toUpperCase()) {
                case 'AFRICASTALKING':
                    sent = yield sendAfricasTalkingSms(phone, options.message, settings);
                    break;
                case 'TWILIO':
                    sent = yield sendTwilioSms(phone, options.message, settings);
                    break;
                default:
                    console.log(`Unknown SMS provider: ${settings.smsProvider}`);
                    if (logId)
                        yield (0, communicationLogService_1.updateCommunicationLogStatus)(logId, 'FAILED', `Unknown provider: ${settings.smsProvider}`);
                    return false;
            }
            if (logId)
                yield (0, communicationLogService_1.updateCommunicationLogStatus)(logId, sent ? 'SENT' : 'FAILED', sent ? undefined : 'Provider returned failure');
            return sent;
        }
        catch (error) {
...
function broadcastNotification(userIds_1, title_1, message_1) {
    return __awaiter(this, arguments, void 0, function* (userIds, title, message, type = 'INFO') {
        try {
            const result = yield prisma_1.prisma.notification.createMany({
