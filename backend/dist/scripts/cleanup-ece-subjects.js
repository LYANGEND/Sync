...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const allEce = yield prisma.subject.findMany({
        ...
        const codeToId = {};
        allEce.forEach((s) => { codeToId[s.code] = s.id; });
        // 2. Perform merges
        const codesToDelete = Object.keys(MERGE_MAP);
        ...
        for (const [oldCode, keepCode] of Object.entries(MERGE_MAP)) {
            ...
            if (!keepId) {
                console.log(`  SKIP ${oldCode} → ${keepCode} — target not found`);
                continue;
            }
            // Move topics
            const topicResult = yield prisma.topic.updateMany({
                where: { subjectId: oldId },
                data: { subjectId: keepId },
            });
            totalTopicsMoved += topicResult.count;
            // Move lesson plans
            const lpResult = yield prisma.lessonPlan.updateMany({
                where: { subjectId: oldId },
                data: { subjectId: keepId },
            });
            totalLessonPlansMoved += lpResult.count;
            // Move assessments
            const assResult = yield prisma.assessment.updateMany({
                where: { subjectId: oldId },
                data: { subjectId: keepId },
            });
            ...
            try {
                yield prisma.$executeRawUnsafe(`DELETE FROM "class_subject_teachers" WHERE "subjectId" = $1`, oldId);
            }
            catch ( /* table may not exist */_a) { /* table may not exist */ }
            // Move student grades (StudentGrade)
            try {
                yield prisma.studentGrade.updateMany({
                    where: { subjectId: oldId },
                    data: { subjectId: keepId },
                });
            }
            catch ( /* may not exist or unique constraint */_b) { /* may not exist or unique constraint */ }
            // Move virtual classrooms
            try {
                yield prisma.virtualClassroom.updateMany({
                    where: { subjectId: oldId },
                    data: { subjectId: keepId },
                });
            }
            catch ( /* nullable field */_c) { /* nullable field */ }
        ...
        for (const [oldCode, keepCode] of Object.entries(MERGE_MAP)) {
            ...
            const classesWithOld = yield prisma.class.findMany({
        ...
        if (idsToDelete.length > 0) {
            const deleteResult = yield prisma.subject.deleteMany({
        ...
        const remaining = yield prisma.subject.findMany({
            where: { code: { startsWith: 'ECE-' } },
            select: { code: true, name: true },
            orderBy: { code: 'asc' },
        });
        console.log(`\n=== FINAL ECE SUBJECTS (${remaining.length}) ===`);
        remaining.forEach((s) => console.log(`  ${s.code.padEnd(12)} ${s.name}`));
        const topicCount = yield prisma.topic.count({
