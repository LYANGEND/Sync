  ...
  const formatResponse = (content: string) => {
    ...
    while (i < lines.length) {
      ...
      if (trimmed.startsWith('```')) {
        ...
        i++; // skip closing ```
        elements.push(
          <div key={elements.length} className="ds-surface my-2 overflow-hidden">
            {lang && <div className="bg-[var(--surface-muted)] px-3 py-1 text-xs font-mono text-[var(--text-secondary)] uppercase tracking-wider">{lang}</div>}
            <pre role="region" aria-label="Advisor code example" tabIndex={0} className="bg-[var(--bg-primary)] px-3 py-2 overflow-x-auto text-sm font-mono text-[var(--text-secondary)] leading-relaxed">
              {codeLines.join('\n')}
            </pre>
          </div>
        );
      ...
      if (trimmed.startsWith('|') && trimmed.endsWith('|') && trimmed.includes('|')) {
        ...
        if (dataRows.length > 0) {
          ...
          elements.push(
            <div key={elements.length} className="ds-table-container my-3 overflow-x-auto" role="region" aria-label="Advisor response table" tabIndex={0}>
              <table className="ds-table">
                <thead>
                  <tr>
                    {headerCells.map((cell, ci) => (
                      <th key={ci} scope="col" className="whitespace-nowrap">
      ...
      if (trimmed.startsWith('> ')) {
        ...
        elements.push(
          <blockquote key={elements.length} className="my-2 border-l-4 border-[var(--border-color)] pl-3 py-1 bg-[var(--surface-muted)]">
    ...
    <div className="ds-page">
      {!embedded && (
        <div className="ds-page-header">
          <div>
          <h1 className="ds-page-title">AI Financial Advisor</h1>
          <p className="ds-page-subtitle">AI-powered financial analysis and recommendations</p>
          </div>
        </div>
      )}
      ...
      <div className="ds-section">
        <button
          ...
          aria-controls={`${advisorId}-insights`}
        ...
        {showInsights && (
          <div id={`${advisorId}-insights`} className="space-y-4">
            {insightsLoading ? (
              <div role="status" className="ds-surface flex items-center justify-center py-12">
              ...
              <div role="alert" className="ds-card">
                <div className="text-center">
                  ...
                  <button
                    ...
                    className="ds-button-outline mx-auto"
                  >
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  {/* Health Score Card */}
                  <div className="ds-card flex flex-col items-center justify-center">
                    ...
                    <div className="ds-card">
                      ...
                      <div className="space-y-2">
                        {insights.criticalAlerts.map((alert, i) => (
                          <div key={i} className={`ds-alert ${getSeverityColor(alert.severity)}`}>
                    ...
                    <div className="ds-card">
                      ...
                      <div className="space-y-2">
                        {insights.recommendations.map((rec, i) => (
                          <div key={i} className="ds-surface-muted p-4">
                            ...
                            <span className="ds-badge ds-badge-success mt-1">
                              Impact: {rec.impact}
                            </span>
                          </div>
                        ))}
      ...
      <div className="ds-section">
        <button
          ...
          aria-expanded={showPowerFeatures}
          aria-controls={`${advisorId}-intelligence`}
          className="ds-button-ghost flex-wrap"
        >
          <Zap size={18} className="text-purple-500" />
          Finance Intelligence Suite
          <span className="ds-badge ds-badge-neutral">10 AI Powers</span>
          {showPowerFeatures ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
        </button>

        {showPowerFeatures && (
          <div id={`${advisorId}-intelligence`} className="ds-surface min-w-0 overflow-hidden">
            {/* Quick Action Buttons */}
            <div className="p-4 border-b border-[var(--border-color)]">
              <Tabs id={`${advisorId}-power`} label="Finance intelligence tools" value={activePowerTab}
                items={[
                  { id: 'forecast', label: 'Cash Flow Forecast' },
                  { id: 'compliance', label: 'Compliance Check' },
                  { id: 'allocator', label: 'Allocate Payments' },
                  { id: 'budget', label: 'Budget vs Actual' },
                  { id: 'audit', label: 'Audit Trail' },
                  { id: 'reports', label: 'Reports & Export' },
                ]}
                onChange={tab => {
                  setActivePowerTab(tab as PowerTab);
                  if (tab === 'forecast' && !cashFlowForecast) loadCashFlowForecast();
                  if (tab === 'compliance' && !complianceStatus) loadComplianceStatus();
                  if ((tab === 'budget' || tab === 'audit') && !rawSnapshot) loadRawSnapshot();
                  if (tab === 'reports') { loadSavedReports(); loadAcademicTerms(); }
                }}
              />
            </div>

            {/* Tab Content */}
            {(['forecast', 'compliance', 'allocator', 'budget', 'audit', 'reports'] as PowerTab[]).filter(tab => tab !== activePowerTab).map(tab => (
              <div key={tab} hidden role="tabpanel" id={`${advisorId}-power-panel-${tab}`} aria-labelledby={`${advisorId}-power-tab-${tab}`} />
            ))}
            <div className="p-4 sm:p-6" role="tabpanel" id={`${advisorId}-power-panel-${activePowerTab}`} aria-labelledby={`${advisorId}-power-tab-${activePowerTab}`} tabIndex={0}>
              ...
              {activePowerTab === 'forecast' && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={loadCashFlowForecast} disabled={forecastLoading} className="ds-button-ghost">
                  ...
                  {forecastLoading ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <button onClick={loadCashFlowForecast} className="ds-button-primary">
                        Generate Forecast
                      </button>
                    </div>
                  ) : (
              ...
              {activePowerTab === 'compliance' && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={loadComplianceStatus} disabled={complianceLoading} className="ds-button-ghost">
                  ...
                  {complianceLoading ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <button onClick={loadComplianceStatus} className="ds-button-primary">
                      ...
                      {complianceStatus.alerts?.length > 0 && (
                        <div className="space-y-2">
                          {complianceStatus.alerts.map((alert, i) => (
                            <div key={i} className="ds-alert ds-badge-warning">
              ...
              {activePowerTab === 'allocator' && (
                <div>
                  ...
                  <div className="ds-surface-muted p-4 mb-4">
                  ...
                  {!allocationResult ? (
                    ...
                    <div className="space-y-3">
                      <div className={`ds-card ${allocationResult.allocated > 0 ? 'border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-900/20' : 'border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20'}`}>
                      ...
                      {allocationResult.results?.length > 0 && (
                        <div>
                          ...
                          <div className="space-y-1 max-h-48 overflow-y-auto" role="region" aria-label="Payment allocation details" tabIndex={0}>
                            {allocationResult.results.map((r: any, i: number) => (
                              <div key={i} className="ds-surface-muted flex flex-wrap items-center justify-between gap-2 px-3 py-2 text-sm">
              ...
              {activePowerTab === 'budget' && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={loadRawSnapshot} disabled={snapshotLoading} className="ds-button-ghost">
                  ...
                  {snapshotLoading ? (
                    ...
                    <div className="space-y-4 max-h-96 overflow-y-auto" role="region" aria-label="Budget utilization" tabIndex={0}>
                      {rawSnapshot.budgets.map((budget: any, bi: number) => (
                        <div key={bi} className="ds-surface overflow-hidden">
              ...
              {activePowerTab === 'audit' && (
                <div>
                  ...
                  {snapshotLoading ? (
                    ...
                    <div className="space-y-1.5 max-h-80 overflow-y-auto" role="region" aria-label="Financial audit activity" tabIndex={0}>
                      {rawSnapshot.recentFinancialActivity.map((log: any, i: number) => (
                        <div key={i} className="ds-surface-muted flex items-start gap-3 px-3 py-2.5">
              ...
              {activePowerTab === 'reports' && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <button
                        onClick={exportPDF}
                        disabled={!insights}
                        className="ds-button-outline flex-col"
                      >
                  ...
                  <div className="border-t border-slate-200 dark:border-slate-700 pt-5">
                    ...
                    <div className="space-y-4">
                      <div>
                        <label htmlFor={`${advisorId}-report-title`} className="ds-label mb-2">Report Title *</label>
                        <input
                          id={`${advisorId}-report-title`}
                          type="text"
                          value={reportTitle}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label htmlFor={`${advisorId}-report-type`} className="ds-label mb-2">Report Type</label>
                          <select
                            id={`${advisorId}-report-type`}
                        ...
                        <div>
                          <label htmlFor={`${advisorId}-report-term`} className="ds-label mb-2">Link to Term (optional)</label>
                          <select
                            id={`${advisorId}-report-term`}
                  ...
                  <div className="border-t border-slate-200 dark:border-slate-700 pt-5">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button onClick={loadSavedReports} disabled={reportsLoading} className="ds-button-ghost">
                    ...
                    {reportsLoading ? (
                      ...
                      <div className="space-y-2 max-h-64 overflow-y-auto" role="region" aria-label="Saved financial reports" tabIndex={0}>
                        {savedReports.map((r: any) => (
                          <div key={r.id} className="ds-surface-muted flex flex-wrap items-center justify-between gap-2 px-4 py-3">
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-slate-800 dark:text-white truncate">{r.title}</p>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="ds-badge ds-badge-neutral">{r.reportType?.replace(/_/g, ' ')}</span>
      ...
      <div className="ds-surface overflow-hidden">
        {/* Chat Header */}
        <div
          className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-[var(--border-color)]"
        >
          <button className="ds-button-ghost min-w-0 text-left" onClick={() => setShowChat(!showChat)} aria-expanded={showChat} aria-controls={`${advisorId}-chat`}>
            <Coins size={18} aria-hidden="true" className="shrink-0" />
            <span className="min-w-0">
              <span className="block font-semibold">Finance AI Advisor</span>
              <span className="ds-helper block break-words">
            ...
            {showChat ? <ChevronDown size={18} aria-hidden="true" /> : <ChevronUp size={18} aria-hidden="true" />}
          </button>
          <div className="ds-actions">
            <button
              onClick={(e) => { e.stopPropagation(); setShowHistory(!showHistory); setShowChat(true); }}
              className="ds-button-secondary"
              aria-expanded={showHistory && showChat}
              aria-controls={`${advisorId}-history`}
              title="Conversation History"
            >
              <History size={13} /> History
              {conversations.length > 0 && (
                <span className="ds-badge ds-badge-neutral">{conversations.length}</span>
              )}
            </button>
            <button
        ...
        {showChat && (
          <div id={`${advisorId}-chat`} className="flex flex-col md:flex-row" style={{ height: showHistory || messages.length > 0 ? '500px' : 'auto' }}>
            {/* Conversation History Sidebar */}
            {showHistory && (
              <div id={`${advisorId}-history`} className="w-full md:w-64 max-h-64 md:max-h-none border-r border-[var(--border-color)] flex flex-col bg-[var(--surface-muted)] flex-shrink-0">
                  ...
                  {conversationsLoading ? (
                    ...
                    <div className="py-1">
                      {conversations.map(conv => (
                        <div
                          key={conv.id}
                          className={`group px-3 py-2.5 border-l-2 ${
                        ...
                        >
                          {renamingId === conv.id ? (
                            ...
                            <button className="ds-button-ghost w-full flex-col items-start text-left" onClick={() => loadConversation(conv.id)} aria-pressed={activeConversationId === conv.id}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {suggestedQuestions.map((q, i) => (
                    <button
                      ...
                      className="ds-button-outline justify-start text-left"
                    >
            ...
            <div className="border-t border-slate-200 dark:border-slate-700 p-3">
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  className="ds-textarea flex-1 min-h-11 resize-none"
                  aria-label="Question for the financial advisor"
                  rows={1}
                ...
                <button
                  ...
                  className="ds-button-primary flex-shrink-0"
              ...
              {messages.length > 0 && (
                <button
                  ...
                  className="ds-button-destructive mt-2"
                >
